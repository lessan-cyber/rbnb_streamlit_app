...
from .schemas.chatschemas import ChatRequest, ChatResponse, ExtractedInfo, Message, ListingResult
...
from .tools.booking_tools import update_booking_parameters
from .tools.search_tools import search_listings, search_tool_schema
from .tools import available_tools, update_booking_tool_schema
...
async def chat_endpoint(request: ChatRequest):
    ...
    logger.info(f"📩 NEW REQUEST | Session: {request.session_id} | Message: '{request.message}'")
    model = await connection_to_gemini() # Assuming this function returns the initialized model
    if not model:
    ...
    try:
        ...
        response = await model.generate_content_async(
            ...
            tools=[update_booking_tool_schema, search_tool_schema] # List of tool schemas
        )
            ...
            if function_name in available_tools:
                ...
                try:
                    ...
                    if function_name == "update_booking_parameters":
                        logger.info(f"✅ FUNCTION SUCCESS | {function_name} | Updated: {function_result_payload}")
                        print(f"Processing result for {function_name}: {function_result_payload}")
                        if isinstance(function_result_payload, dict):
                             # Update the state variable for the final response
                             updated_info_for_response = ExtractedInfo(**function_result_payload)
                             # Content for Gemini is the dict itself
                             tool_result_content_for_gemini = function_result_payload
                        else:
                            print(f"Warning: {function_name} did not return a dictionary.")
                            # Keep original info state, send empty dict back to Gemini? Or error?
                            tool_result_content_for_gemini = {"error": "Invalid tool result format"}
                        ...
                        if isinstance(function_result_payload, list):
                            ...
                            for item in function_result_payload:
                                if isinstance(item, dict):
                                    try:
                                        ...
                                        logger.warning(f"Skipping search result due to parsing error: {parse_error}")
                                else:
                                    logger.warning(f"Item in search result is not a dictionary: {type(item)}")
                            
                            search_results_for_response = validated_results
                            
                            # Log the success with more detail
                            if validated_results:
                                logger.info(f"✅ SEARCH SUCCESS | Found: {len(validated_results)} listings | " + 
                                            f"Sample: {validated_results[0].city}, {validated_results[0].title}")
                            else:
                                logger.warning(f"⚠️ SEARCH RETURNED EMPTY | Args: {function_args}")
                            ...
                            for item in validated_results[:3]:  # Limit to first 3 listings
                                listing_summary = {
                                    "title": item.title,
                                    "city": item.city,
                                    "price_per_night": f"${item.price_per_night:.2f}" if item.price_per_night else "Price not available",
                                    "max_guests": f"Up to {item.max_guests} guests" if item.max_guests else "Guest count not specified",
                                    "bedrooms": f"{item.bedrooms} bedrooms" if item.bedrooms else "Bedroom count not available",
                                    "description": item.description[:100] + "..." if item.description and len(item.description) > 100 else item.description
                                }
                                tool_result_content_for_gemini["listings_summary"].append(listing_summary)

                        else:
                            print(f"Warning: {function_name} did not return a list.")
                            search_results_for_response = [] # Send empty list to frontend
                            tool_result_content_for_gemini = {"status": "success", "listings_found_count": 0, "listings_summary": []}

                    else:
                        # Handle future tools if needed
                        print(f"Processing result for unhandled tool: {function_name}")
                        tool_result_content_for_gemini = function_result_payload if function_result_payload else {}

                    gemini_history_content.append({'role': 'model', 'parts': [response_part]}) # Append model's request part
                    ...
                    if not second_response.candidates[0].content.parts:
                         raise ValueError("Gemini response candidate (pass 2) missing parts.")
                    ...
                    logger.error(f"❌ FUNCTION FAILED | {function_name} | Error: {tool_exec_error}")
                    traceback.print_exc()
                    ...
                    search_results_for_response = None # Ensure no partial results are sent
    ...
    final_history = chat_history + [
        ...
        Message(role="assistant", content=ai_natural_response) # Save final AI text response
    ]
    print("Saving final conversation state...")
    # Use the potentially updated 'updated_info_for_response' Pydantic model
