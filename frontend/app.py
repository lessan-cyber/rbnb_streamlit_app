...
for message in st.session_state.messages:
    with st.chat_message(message["role"]):
        ...
        if (
            ...
            and "search_results" in message
            and message["search_results"]
        ):
            st.write(
                "_(Suggested listings from this message)_"
            )  # Indicate these are from history
            ...
            for i, listing in enumerate(message["search_results"]):
                ...
                with cols[col_index]:
                    with st.container(border=True):
                        ...
                        details = []
                        if listing.get("max_guests"):
                            details.append(f"Sleeps {listing['max_guests']}")
                        if listing.get("bedrooms"):
                            details.append(f"{listing['bedrooms']} BR")
                        st.caption(" | ".join(details))
                        if listing.get("price_per_night") is not None:
                            st.write(f"**${listing['price_per_night']:.2f}** / night")
            if num_listings > max_cols:
                st.caption(
                    f"_(Displaying {max_cols} of {num_listings} listings from this message)_"
                )
...
if prompt := st.chat_input("How can I help with your booking?"):
    ...
    ai_response_text = (
        "Error: Could not get response from backend."  # Default error message
    )
    ...
    try:
        ...
        ai_response_text = response_data.get(
            "response", "Sorry, I received an empty response."
        )

        # <-- NEW: Extract search results if they exist -->
        search_results_list = response_data.get(
            "search_results"
        )  # This will be None or a list of dicts
        ...
        if updated_info_dict is not None:
            st.session_state.booking_info = (
                updated_info_dict  # Update local copy if needed
            )
    ...
    assistant_message_data = {"role": "assistant", "content": ai_response_text}
            ...
            if num_results > 0:
                ...
                for i, listing in enumerate(search_results_list):
                    ...
                    with cols[col_index]:  # Place each listing in a column
                        with st.container(
                            border=True
                        ):  # Use container with border for card effect
                            ...
                            if listing.get("price_per_night") is not None:
                                st.write(
                                    f"**${listing['price_per_night']:.2f}** / night"
                                )
                            else:
                                st.write("Price N/A")
                            ...
                            st.button(
                                "Select",
                                key=f"select_{listing.get('id')}_{st.session_state.session_id}",
                            )  # Use unique key

                if num_results > max_cols_display:
                    st.caption(
                        f"Showing {max_cols_display} of {num_results} suggestions."
                    )
